package com.phuc.payment.service.impl;
...
import com.phuc.payment.exception.AppException;
import com.phuc.payment.exception.ErrorCode;
import com.phuc.payment.mapper.StripeMapper;
import com.phuc.payment.repository.SessionRepository;
import com.phuc.payment.repository.StripeChargeRepository;
import com.phuc.payment.repository.StripeSubscriptionRepository;
...
public class StripeServiceImpl implements StripeService {
      ...
      public StripeChargeResponse processCharge(StripeChargeRequest request) {
            ...
            try {
                  ...
                  if (charge.getPaid()) {
                        ...
                        log.info("Payment succeeded for user {} ({}) with orderId {}", username, email, request.getOrderId());
                  } else {
                        stripeCharge.setSuccess(false);
                        log.warn("Stripe charge reported as unpaid for user {} ({}) with orderId {}", username, email, request.getOrderId());
                  }
      ...
      public SessionResponse createChargeSession(ChargeSessionRequest request) {
            com.phuc.payment.entity.Session chargeSession = stripeMapper.toSession(request);
      ...
      public SessionResponse createSubscriptionSession(SubscriptionSessionRequest request) {
            ...
            try {
                  ...
                  com.phuc.payment.entity.Session session = com.phuc.payment.entity.Session.builder()
